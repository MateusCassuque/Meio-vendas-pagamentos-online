        ...
        <v-container fluid>            
                <v-row class="mx-auto">
                    <v-card class="col-md-4 my-4 mx-auto"  max-width="350" v-for="product in produtos" :key="product._id">
                        ...
                        <v-card-subtitle v-if="!product.type" class="pt-4"> <span>Preço: </span> {{ product.preco }} AOA </v-card-subtitle>
                        
                        <v-card-text v-if="!product.type" class="text--primary">
                            <div>
                                <p><span>Quantidade: </span> <span> {{ product.quantidade }} </span></p>
                            </div>
                            <div>
                                <span>Tipo de Produto:</span> <span>Físico</span>
                            </div>
                        ...
                        <v-divider v-show="product.type" class="mx-4"></v-divider>
                        
                        <v-card-actions v-if="!product.type">
                            <v-btn color="orange" variant="text" :to="`/product/${product._id}`"> Ver Detalhes </v-btn>
                            <v-btn color="orange" variant="text"> Comprar </v-btn>
                        </v-card-actions>
